...
open class IOStickyHeaderFlowLayout: UICollectionViewFlowLayout {
    ...
    open override func initialLayoutAttributesForAppearingSupplementaryElement(ofKind elementKind: String, at elementIndexPath: IndexPath) -> UICollectionViewLayoutAttributes? {
        ...
        if var frame = attributes?.frame, let referenceHeight = self.parallaxHeaderReferenceSize?.height {
    ...
    open override func layoutAttributesForElements(in rect: CGRect) -> [UICollectionViewLayoutAttributes]? {
        ...
        if let referenceHeight = self.parallaxHeaderReferenceSize?.height {
            adjustedRec.origin.y -= referenceHeight
        }
        ...
        guard let referenceSize = self.parallaxHeaderReferenceSize  else {
            return allItems
        }

        if visibleParallaxHeader && !CGSize.zero.equalTo(referenceSize) {
